...
GameSession::GameSession(const std::string &levelfile_, Savegame &savegame,
                         Statistics *statistics)
    : reset_button(false), level(), old_level(),
      statistics_backdrop(
          Surface::create("images/engine/menu/score-backdrop.png")),
      scripts(), currentsector(nullptr), levelnb(), pause_menu_frame(),
      end_sequence(0), game_pause(false),
      speed_before_pause(ScreenManager::current()->get_speed()),
      levelfile(levelfile_), reset_sector(), reset_pos(), newsector(),
      newspawnpoint(), best_level_statistics(statistics), m_savegame(savegame),
      capture_demo_stream(0), capture_file(), playback_demo_stream(0),
      demo_controller(0), play_time(0), levelintro_shown(false),
      coins_at_start(), bonus_at_start(), max_fire_bullets_at_start(),
      max_ice_bullets_at_start(), active(false), end_seq_started(false) {
  if (restart_level() != 0)
    throw std::runtime_error("Initializing the level failed.");
}

void GameSession::reset_level() {
...
int GameSession::restart_level(bool after_death) {
  PlayerStatus *currentStatus = m_savegame.get_player_status();
  ...
  try {
    ...
    if (!reset_sector.empty()) {
      ...
      if (!currentsector) {
        ...
        msg << "Couldn't find sector '" << reset_sector
            << "' for resetting tux.";
...
int GameSession::get_demo_random_seed(const std::string &filename) const {
...
void GameSession::draw(DrawingContext &context) {
...
void GameSession::draw_pause(DrawingContext &context) {
...
void GameSession::setup() {
  ...
  int total_stats_to_be_collected = level->stats.total_coins +
                                    level->stats.total_badguys +
                                    level->stats.total_secrets;
  if ((!levelintro_shown) && (total_stats_to_be_collected > 0)) {
    ...
    ScreenManager::current()->push_screen(std::unique_ptr<Screen>(
        new LevelIntro(level.get(), best_level_statistics,
                       m_savegame.get_player_status())));
  }
...
void GameSession::update(float elapsed_time) {
  ...
  if (InputManager::current()->get_controller()->pressed(
          Controller::CHEAT_MENU) &&
  ...
  // update sounds
  if (currentsector->camera)
    SoundManager::current()->set_listener_position(
        currentsector->camera->get_center());
...
void GameSession::finish(bool win) {
  if (end_seq_started)
    return;
...
void GameSession::respawn(const std::string &sector,
                          const std::string &spawnpoint) {
  newsector = sector;
  newspawnpoint = spawnpoint;
}

void GameSession::set_reset_point(const std::string &sector,
                                  const Vector &pos) {
  reset_sector = sector;
  reset_pos = pos;
}
...
void GameSession::start_sequence(Sequence seq) {
  ...
  if (seq == SEQ_ENDSEQUENCE) {
    ...
    log_warning << "Unknown sequence '" << (int)seq << "'. Ignoring."
                << std::endl;
    return;
  }
...
/* (Status): */
void GameSession::drawstatus(DrawingContext &context) {
  m_savegame.get_player_status()->draw(context);

  // draw level stats while end_sequence is running
  if (end_sequence) {
    level->stats.draw_endseq_panel(context, best_level_statistics,
                                   statistics_backdrop);
  }
}
