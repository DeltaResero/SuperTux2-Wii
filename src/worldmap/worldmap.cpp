...
WorldMap::WorldMap(const std::string &filename, Savegame &savegame,
                   const std::string &force_spawnpoint_)
    : tux(), m_savegame(savegame), tileset(nullptr), camera_offset(), name(),
      music(), init_script(), game_objects(), solid_tilemaps(),
      passive_message_timer(), passive_message(), map_filename(), levels_path(),
      special_tiles(), levels(), sprite_changes(), spawn_points(),
      teleporters(), total_stats(), worldmap_table(), scripts(),
      ambient_light(1.0f, 1.0f, 1.0f, 1.0f),
      force_spawnpoint(force_spawnpoint_), in_level(false), pan_pos(),
      panning(false) {
  ...
  if (SQ_FAILED(sq_setdelegate(global_vm, -2)))
    throw scripting::SquirrelError(global_vm,
                                   "Couldn't set worldmap_table delegate");
...
void WorldMap::add_object(GameObjectPtr object) {
...
void WorldMap::try_expose(const GameObjectPtr &object) {
...
void WorldMap::try_unexpose(const GameObjectPtr &object) {
  auto object_ = dynamic_cast<ScriptInterface *>(object.get());
  if (object_ != NULL) {
    HSQUIRRELVM vm = scripting::global_vm;
...
void WorldMap::move_to_spawnpoint(const std::string &spawnpoint, bool pan) {
    ...
    if (!level_.get("sector", sector)) {
      ...
      while (iter.next()) {
        if (iter.get_key() == "tilemap") {
          ...
          auto level =
              std::make_shared<LevelTile>(levels_path, iter.as_mapping());
          ...
          auto sprite_change =
              std::make_shared<SpriteChange>(iter.as_mapping());
...
void WorldMap::load_level_information(LevelTile &level) {
...
Vector WorldMap::get_next_tile(Vector pos, const Direction &direction) const {
  switch (direction) {
  case D_WEST:
    pos.x -= 1;
    break;
...
bool WorldMap::path_ok(const Direction &direction, const Vector &old_pos,
                       Vector *new_pos) const {
  *new_pos = get_next_tile(old_pos, direction);

  if (!(new_pos->x >= 0 && new_pos->x < get_width() && new_pos->y >= 0 &&
        new_pos->y < get_height())) { // New position is outsite the tilemap
    return false;
  } else { // Check if the tile allows us to go to new_pos
...
void WorldMap::finished_level(Level *gamelevel) {
  ...
  if (old_level_state != level->solved) {
    ...
    if (dirdata == Tile::WORLDMAP_CNSE || dirdata == Tile::WORLDMAP_CNSW ||
        dirdata == Tile::WORLDMAP_CNEW || dirdata == Tile::WORLDMAP_CSEW ||
        dirdata == Tile::WORLDMAP_CNSEW)
      dir = D_NONE;
    else if (dirdata & Tile::WORLDMAP_NORTH && tux->back_direction != D_NORTH)
      dir = D_NORTH;
    else if (dirdata & Tile::WORLDMAP_SOUTH && tux->back_direction != D_SOUTH)
      dir = D_SOUTH;
    else if (dirdata & Tile::WORLDMAP_EAST && tux->back_direction != D_EAST)
      dir = D_EAST;
    else if (dirdata & Tile::WORLDMAP_WEST && tux->back_direction != D_WEST)
      dir = D_WEST;
...
Vector WorldMap::get_camera_pos_for_tux() const {
...
void WorldMap::clamp_camera_position(Vector &c) const {
  if (c.x < 0)
    c.x = 0;
  if (c.y < 0)
    c.y = 0;
...
void WorldMap::update(float delta) {
  if (!in_level && !MenuManager::instance().is_active()) {
    ...
    if (teleporter &&
        (teleporter->automatic || (enter_level && (!tux->is_moving())))) {
      enter_level = false;
      if (!teleporter->worldmap.empty()) {
        change(teleporter->worldmap, teleporter->spawnpoint);
      } else {
        // TODO: an animation, camera scrolling or a fading would be a nice
        // touch
    ...
    auto level = at_level();
    if (level && (level->auto_play) && (!level->solved) &&
        (!tux->is_moving())) {
      enter_level = true;
    ...
    if (enter_level && !tux->is_moving()) {
      ...
      if (!level_) {
        ...
        if (!(tile_data & (Tile::WORLDMAP_NORTH | Tile::WORLDMAP_SOUTH |
                           Tile::WORLDMAP_WEST | Tile::WORLDMAP_EAST))) {
          log_warning << "Player at illegal position " << tux->get_tile_pos().x
                      << ", " << tux->get_tile_pos().y << " respawning."
                      << std::endl;
          move_to_spawnpoint("main");
          return;
        }
        log_warning << "No level to enter at: " << tux->get_tile_pos().x << ", "
                    << tux->get_tile_pos().y << std::endl;
        return;
      }
      ...
      if (level_->pos == tux->get_tile_pos()) {
        try {
          ...
          std::string levelfile =
              FileSystem::join(levels_path, level_->get_name());

          // update state and savegame
          save_state();
          ScreenManager::current()->push_screen(
              std::unique_ptr<Screen>(
                  new GameSession(levelfile, m_savegame, &level_->statistics)),
...
int WorldMap::tile_data_at(const Vector &p) const {
  int dirs = 0;
...
int WorldMap::available_directions_at(const Vector &p) const {
  return tile_data_at(p) & Tile::WORLDMAP_DIR_MASK;
}

LevelTile *WorldMap::at_level() const {
  for (const auto &level : levels) {
...
SpecialTile *WorldMap::at_special_tile() const {
...
SpriteChange *WorldMap::at_sprite_change(const Vector &pos) const {
...
Teleporter *WorldMap::at_teleporter(const Vector &pos) const {
...
void WorldMap::draw(DrawingContext &context) {
  if (int(get_width() * 32) < SCREEN_WIDTH ||
      int(get_height() * 32) < SCREEN_HEIGHT)
  ...
  int py = y * 32;
  const int W = 4;
  if (data & Tile::WORLDMAP_NORTH)    context.draw_filled_rect(Rect(px + 16-W,
  py       , px + 16+W, py + 16-W), Color(0.2f, 0.2f, 0.2f, 0.7f),
  LAYER_FOREGROUND1 + 1000); if (data & Tile::WORLDMAP_SOUTH)
  context.draw_filled_rect(Rect(px + 16-W, py + 16+W, px + 16+W, py + 32  ),
  Color(0.2f, 0.2f, 0.2f, 0.7f), LAYER_FOREGROUND1 + 1000); if (data &
  Tile::WORLDMAP_EAST)     context.draw_filled_rect(Rect(px + 16+W, py + 16-W,
  px + 32  , py + 16+W), Color(0.2f, 0.2f, 0.2f, 0.7f), LAYER_FOREGROUND1 +
  1000); if (data & Tile::WORLDMAP_WEST)     context.draw_filled_rect(Rect(px ,
  py + 16-W, px + 16-W, py + 16+W), Color(0.2f, 0.2f, 0.2f, 0.7f),
  LAYER_FOREGROUND1 + 1000); if (data & Tile::WORLDMAP_DIR_MASK)
  context.draw_filled_rect(Rect(px + 16-W, py + 16-W, px + 16+W, py + 16+W),
  Color(0.2f, 0.2f, 0.2f, 0.7f), LAYER_FOREGROUND1 + 1000); if (data &
  Tile::WORLDMAP_STOP)     context.draw_filled_rect(Rect(px + 4   , py + 4   ,
  px + 28  , py + 28  ), Color(0.2f, 0.2f, 0.2f, 0.7f), LAYER_FOREGROUND1 +
  1000);
  }
  }
  */
...
void WorldMap::draw_status(DrawingContext &context) {
  context.push_transform();
  ...
  if (!tux->is_moving()) {
    for (const auto &level : levels) {
      if (level->pos == tux->get_tile_pos()) {
        context.draw_text(
            Resources::normal_font, level->title,
        ...
        /*
          ...
          Vector pos = Vector(SCREEN_WIDTH - picture->get_width(), SCREEN_HEIGHT
          - picture->get_height()); context.push_transform();
    ...
    for (const auto &special_tile : special_tiles) {
      if (special_tile->pos == tux->get_tile_pos()) {
        ...
        if (!special_tile->map_message.empty() &&
            !special_tile->passive_message)
          context.draw_text(
              Resources::normal_font, special_tile->map_message,
              Vector(SCREEN_WIDTH / 2,
    ...
    if (teleporter && (!teleporter->message.empty())) {
      ...
      context.draw_text(Resources::normal_font, teleporter->message, pos,
                        ALIGN_CENTER, LAYER_FOREGROUND1,
                        WorldMap::teleporter_message_color);
    }
  }
  ...
  if (passive_message_timer.started())
    context.draw_text(
        Resources::normal_font, passive_message,
        Vector(SCREEN_WIDTH / 2,
               SCREEN_HEIGHT - Resources::normal_font->get_height() - 60),
        ALIGN_CENTER, LAYER_FOREGROUND1, WorldMap::message_color);
...
void WorldMap::setup() {
  ...
  ScreenManager::current()->set_screen_fade(
      std::unique_ptr<ScreenFade>(new FadeIn(1)));
  ...
  try {
    std::string script_path =
        FileSystem::find(FileSystem::join(levels_path, "default.nut"));
...
void WorldMap::set_levels_solved(bool solved, bool perfect) {
...
void WorldMap::load_state() {
  ...
  try {
    ...
    if (!(tile_data & (Tile::WORLDMAP_NORTH | Tile::WORLDMAP_SOUTH |
                       Tile::WORLDMAP_WEST | Tile::WORLDMAP_EAST))) {
      log_warning << "Player at illegal position " << p.x << ", " << p.y
                  << " respawning." << std::endl;
      move_to_spawnpoint("main");
    }
...
size_t WorldMap::level_count() const { return levels.size(); }

size_t WorldMap::solved_level_count() const {
  size_t count = 0;
  for (const auto &level : levels) {
    if (level->solved)
      count++;
...
WorldMap::run_script(std::istream &in, const std::string &sourcename) {
  ...
  for (ScriptList::iterator i = scripts.begin(); i != scripts.end();) {
    HSQOBJECT &object = *i;
...
float WorldMap::get_width() const {
  float width = 0;
  for (const auto &solids : solid_tilemaps) {
    if (solids->get_width() > width)
      width = solids->get_width();
  }
  return width;
}

float WorldMap::get_height() const {
  float height = 0;
  for (const auto &solids : solid_tilemaps) {
    if (solids->get_height() > height)
      height = solids->get_height();
  }
  return height;
}
